...
z_image=cv2.resize(z_image,(30,30))
...
#placeholders
z=tf.placeholder(dtype=tf.float64,shape=[None,30,30,3])
z=tf.reshape(z,[-1,30,30,3])
...
#    return layer3
    ...
    w4=tf.Variable(tf.random_normal([5,5,3,3],mean=0.01,stddev=0.01,dtype=tf.float64))
    b4=tf.Variable(tf.random_normal([3],mean=0.01,stddev=0.01,dtype=tf.float64))
    conv4=tf.nn.conv2d(layer3,w4,strides=[1,1,1,1],padding='SAME')+b4
    layer4=tf.nn.softmax(conv4)
    # print("Layer 2 shape     " +str(layer2.shape))
    gen_params=[w1,b1,w2,b2,w3,b3,w4,b4]
    return layer4,gen_params
